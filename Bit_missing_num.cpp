 ...
 * then remove all binary numbers that least significant bit is not equal to the removed bit
 */
...
int findMissingInteger(int * array, int n) {
	...
	while (NumOfOne > 0) {
		...
		NumOfOne = (n % (1 << (column + 1))) - (1 << column)+1;
		if (NumOfOne < 0)
			NumOfOne = 0;
...
int main() {
	int a[11] = { 0, 1, 2, 3, 4, 6, 7, 8, 9, 10,11 }; // missing 5
	cout << "array: 0,1,2,3,4,6,7,8,9,10" << endl;
	cout << "missing: " << findMissingInt(a, 11, 0) << endl;
	cout << "missing: " << findMissingInteger(a, 11);
